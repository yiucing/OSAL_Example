 ...
 * LOCAL VARIABLES
 */
static uint32 previousTimerTick = 0;
...
void osalTimeUpdate( void )
{
  ...
  HAL_EXIT_CRITICAL_SECTION(intState);
   ...
   if(tmp != previousTimerTick)
   {
       if(tmp > previousTimerTick)
       {
           elapsedTicks = tmp - previousTimerTick;
       }
       else
       {
           elapsedTicks = tmp + OSAL_MAX_TICKS - previousTimerTick;
       }
      
       // Update OSAL Clock and Timers
       osalClockUpdate( elapsedTicks * OSAL_MSEC_PER_TICK );
       osalTimerUpdate( elapsedTicks * OSAL_MSEC_PER_TICK );

	previousTimerTick = tmp;
    }
}
